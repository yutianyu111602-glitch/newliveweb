...
export class LiquidMetalLayerV2 implements Layer {
  ...
  setAudioFrame(frame: AudioFrame) {
    this.currentAudioBands = frame.bands;
    this.currentAudioEnergy = frame.energy;
  }
  ...
  update(deltaTime: number) {
    ...
    if (this.params.audioReactive && audioBands) {
      ...
      this.material.uniforms.uBrightness.value = this.params.brightness + this.currentAudioEnergy * 0.4;
    } else {
